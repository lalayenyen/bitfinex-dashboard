...
# ==========================================
# V10.2 極度退避版：保護 IP 並找回所有功能
# ==========================================
st.set_page_config(page_title="Bitfinex 智慧戰情室 V10.2", page_icon="💰", layout="wide")
...
def get_data_from_book_safe(symbol):
    try:
        # 只讀取最單純的 Order Book，不帶任何額外參數
...
def display_column(col, title, symbol):
    with col:
        st.header(title)
        asks = get_data_from_book_safe(symbol)
        ...
        if asks:
            ...
            st.markdown(f"""<div style="padding:15px; border-radius:10px; background-color:#f8f9fb; border-left: 5px solid #00d4ff;">
                ...
                <code>隊頭: {frr_sim*100:.4f}% | 年化: {frr_sim*36500:.1f}%</code>
            </div>""", unsafe_allow_html=True)

            # 2. 指標
            best_wall = df.loc[df['掛單量'].idxmax()]
            m1, m2, m3 = st.columns(3)
            m1.metric("1.隊頭基準", f"{frr_sim*100:.4f}%")
            m2.metric("2.最大阻力牆", f"{best_wall['利率']*100:.4f}%")
            m3.metric("3.智慧插針", f"{df['利率'].max()*100:.4f}%")

            # 3. 穩健分析
            st.info(f"💡 **穩健分析**：真正大牆位於 **{best_wall['利率']*100:.4f}%** (總量 {best_wall['掛單量']:,.0f})。")
            ...
            st.warning("🔄 數據冷卻中，請避開頻繁刷新...")

# --- 主佈局 ---
st.title("💰 Bitfinex 智慧戰情室 V10.2")
col_a, col_b = st.columns(2)
display_column(col_a, "🇺🇸 USD (美金)", 'fUSD')
display_column(col_b, "₮ USDT (泰達幣)", 'fUST')

time.sleep(30) # 大幅延長刷新時間，保護 IP
st.rerun()